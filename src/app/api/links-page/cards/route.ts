...
export async function POST(req: Request) {
  ...
  try {
    ...
    const createData = {
      ...result.data,
      order,
      startsAt: result.data.startsAt ? new Date(result.data.startsAt) : (result.data.startsAt ?? undefined),
      endsAt: result.data.endsAt ? new Date(result.data.endsAt) : (result.data.endsAt ?? undefined),
    };
    const card = await LinkCardModel.create(createData);
